...
def snake_segmentation(original, image_array, gamma, nr_of_points, gaussian_sigma, pixel_area, w_edge=2):
    ...
    myo_org_seg = np.where(original < SegmentationLevel.MYO.value, 0, image_array)
    lv_org_seg = np.where(original < SegmentationLevel.LV.value, 0, image_array)

    #return [metrics((lv_org_seg != 0).astype(np.bool), (lv_seg != 0).astype(np.bool), voxel_size),
    #        metrics((myo_org_seg != 0).astype(np.bool), (myo_seg != 0).astype(np.bool), voxel_size)]
   ...
   # if lv_area < 50:
    ...
    return [metrics((lv_org_seg != 0).astype(np.bool), lv_mask, voxel_size),
            metrics((myo_org_seg != 0).astype(np.bool), myo_mask, voxel_size)]
...
# stats = [[0.005,100,1], [0.0002,500,1], [0.00005,1000,1],[0.005,100,2],[0.0002,500,2],[0.00005,1000,2],[0.005,100,3],[0.0002,500,3],[0.00005,1000,3]]
stats = [[0.005, 100, 1]]
original_path = '/Users/antonioneto/Downloads/testing2'
directory_path = '/Users/antonioneto/Downloads/OUTPUT_TESTING'
for SLICE in [1]:  # [1,3,5]:
    ...
    for stat in stats:
        ...
        for filename in os.listdir(directory_path):
            if not filename.__contains__('.nii'):
                continue
            nifti_image_pred = f'{directory_path}/{filename}'  # Reach out in case you need a file to test
            img_nifti = nib.load(nifti_image_pred)

            original = f'{original_path}/{filename}'  # Reach out in case you need a file to test
            original_nifti = nib.load(original)
            try:
                print(filename)
                ...
                background = img_nifti.get_data()[:, :, SLICE]
                original = original_nifti.get_data()[:, :, SLICE]
                print(SLICE)
                pixdim_xyz = img_nifti.header["pixdim"][1:4]
                pixel_area = pixdim_xyz[0] * pixdim_xyz[1]
                # print(pixel_area)
                ...
                if np.any(background == SegmentationLevel.MYO.value) and np.any(
                    ...
                    res = snake_segmentation(original,background, stat[0], stat[1], stat[2], pixel_area)
                    # print(res[3:6])
                    ...
                    original = original_nifti.get_data()[:, :, SLICE - 1]
                    if np.any(background == SegmentationLevel.MYO.value) and np.any(
                            background == SegmentationLevel.LV.value):
                        res = snake_segmentation(original,background, stat[0], stat[1], stat[2], pixel_area)
                        # print(res[3:6])
